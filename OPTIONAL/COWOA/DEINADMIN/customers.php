 ...
 * @version $Id: customers.php for Amazon Pay V2 and COWOA 2023-04-02 11:30:51Z webchills $
 */
  ...
  switch ($action) {
    ...
    default:
      $customers = $db->Execute("SELECT c.customers_id, c.customers_gender, c.customers_firstname,
                                        ...
                                        c.COWOA_account, c.customers_authorization, c.customers_referral, c.customers_secret, customers_amazonpay_ec, customers_paypal_ec
            ...
            <table class="table table-hover">
              <thead>
                <tr class="dataTableHeadingRow">
                  ...
                  <!-- bof Amazon Pay additional column heading in list of customers -->
                  <th class="dataTableHeadingContent text-center">
                    <?php echo (($_GET['list_order'] == 'Amazon-asc' or $_GET['list_order'] == 'Amazon-desc') ? '<span class="SortOrderHeader">' . TABLE_HEADING_AMAZON . '</span>' : TABLE_HEADING_AMAZON); ?><br>
                    <a href="<?php echo zen_href_link(basename($PHP_SELF), zen_get_all_get_params(array('list_order', 'page')) . 'list_order=Amazon-asc', 'NONSSL'); ?>"><?php echo ($_GET['list_order'] == 'Amazon-asc' ? '<span class="SortOrderHeader">Asc</span>' : '<span class="SortOrderHeaderLink">Asc</span>'); ?></a>&nbsp;
                    <a href="<?php echo zen_href_link(basename($PHP_SELF), zen_get_all_get_params(array('list_order', 'page')) . 'list_order=Amazon-desc', 'NONSSL'); ?>"><?php echo ($_GET['list_order'] == 'Amazon-desc' ? '<span class="SortOrderHeader">Desc</span>' : '<span class="SortOrderHeaderLink">Desc</span>'); ?></a>
                  </th>
                  <!-- eof Amazon Pay additional column heading in list of customers -->               
              ...
              <tbody>
                  ...
                  $customers_query_raw = "SELECT c.customers_id, c.customers_lastname, c.customers_firstname, c.customers_email_address, c.customers_group_pricing, c.customers_telephone, c.customers_authorization, c.customers_referral, c.COWOA_account, c.customers_secret, c.customers_amazonpay_ec, customers_paypal_ec,
...
//    zen_redirect(zen_href_link(FILENAME_CUSTOMERS, 'cID=' . $_GET['cID'] . (isset($_GET['page']) ? '&page=' . $_GET['page'] : ''), 'NONSSL'));
                ...
                <!-- bof Amazon Pay additional field in table -->
                <td class="dataTableContent text-center">
                <?php 
                if ($customers->fields['customers_amazonpay_ec'] == 1) { echo 'Amazon Pay'; }
                else 	if ($customers->fields['customers_paypal_ec'] == 1) { echo 'PayPal Express'; }
                else { echo 'Shop';
                }
                ?>
                </td>
                <!-- eof Amazon Pay additional field in table -->
